...
function shouldRetryIgnoredItem(count: number, lastChecked: string): boolean {
  ...
  ztoolkit.log(
    `Auto update debug: shouldRetryIgnoredItem - count=${count}, daysDiff=${daysDiff.toFixed(1)}, requiredDays=${requiredDays}, shouldRetry=${shouldRetry}`,
  )
...
function isItemIgnoredForAutoupdate(itemId: number): boolean {
  ...
  ztoolkit.log(`Auto update debug: Checking if item ${itemId} is ignored. Databases: ${databases.join(', ')}`)

  const ignoredData = getPref('ignoredItems')
  if (!ignoredData) {
    ztoolkit.log(`Auto update debug: No ignoredItems pref found for item ${itemId}`)
    return false
  }

  ztoolkit.log(`Auto update debug: ignoredItems pref data: ${ignoredData.substring(0, 200)}...`)

  let data: IgnoredItemsData
  try {
    data = JSON.parse(ignoredData)
  } catch (error) {
    ztoolkit.log(`Auto update debug: Failed to parse ignoredItems JSON for item ${itemId}:`, error)
    return false
  }
  ...
  for (const database of databases) {
    if (data[database]?.[itemKey]) {
      ...
      ztoolkit.log(
        `Auto update debug: Item ${itemId} in database ${database}: count=${itemData.count}, lastChecked=${itemData.lastChecked}, shouldRetry=${shouldRetry}`,
      )

      if (shouldRetry) {
        checkableDatabases++
        ztoolkit.log(`Auto update debug: Item ${itemId} can be retried for database ${database}`)
      } else {
        blockedDatabases++
        ztoolkit.log(`Auto update debug: Item ${itemId} is blocked for database ${database}`)
      }
    } else {
      // If no ignore data for this database, we can check it
      checkableDatabases++
      ztoolkit.log(`Auto update debug: Item ${itemId} has no ignore data for database ${database} - can check`)
    }
  }
  ...
  ztoolkit.log(
    `Auto update debug: Item ${itemId} final ignore result: ${shouldSkip} (checkable=${checkableDatabases}, blocked=${blockedDatabases}, total=${databases.length})`,
  )
 ...
 */
function isCitationDataOutdated(item: Zotero.Item): boolean {
  const extra = item.getField('extra')
  if (!extra) {
    return true // No citation data at all
  }
  ...
  ztoolkit.log(
    `Auto update debug: Checking outdated data for item ${item.id}, cutoff date: ${cutoffDate.toISOString()}`,
  )
  ...
  for (const database of databases) {
    ...
    if (ignoredItemsData[database]?.[itemKey]) {
      ...
      const shouldRetry = shouldRetryIgnoredItem(itemData.count, itemData.lastChecked)
      if (!shouldRetry) {
        ztoolkit.log(`Auto update debug: Skipping database ${database} for item ${item.id} (blocked by ignored items)`)
        continue
      }
    }
    ...
    for (const line of lines) {
      ...
      if (match) {
        ...
        ztoolkit.log(
          `Auto update debug: Found ${database} data for item ${item.id} dated ${citationDate.toISOString()}`,
        )

        if (citationDate < cutoffDate) {
          ztoolkit.log(`Auto update debug: ${database} data for item ${item.id} is outdated`)
          hasAnyCheckableOutdatedData = true
        } else {
          ztoolkit.log(`Auto update debug: ${database} data for item ${item.id} is recent`)
        }
        found = true
        break
      }
    ...
    if (!found) {
      ztoolkit.log(`Auto update debug: No ${database} data found for item ${item.id} - needs update`)
      hasAnyCheckableOutdatedData = true
    }
  }

  ztoolkit.log(`Auto update debug: Item ${item.id} outdated result: ${hasAnyCheckableOutdatedData}`)
  return hasAnyCheckableOutdatedData
}
...
async function getItemsNeedingUpdate(): Promise<Zotero.Item[]> {
  ...
  for (const item of allItems) {
    ...
    if (isItemIgnoredForAutoupdate(item.id)) {
      ztoolkit.log(`Auto update: Skipping item ${item.id} (ignored for all databases)`)
      ignoredItemCount++
      continue
    }

    if (isCitationDataOutdated(item)) {
...
async function processAutoUpdateQueue(silent: boolean = false) {
  ...
  try {
    ...
    ztoolkit.log('Auto update: Error processing item:', error)

    // Check if it's a rate limit error
    const errorMessage = error instanceof Error ? error.message : String(error)
    if (errorMessage.includes('429') || errorMessage.includes('rate limit')) {
      ztoolkit.log('Auto update: Rate limited, waiting longer...')
...
function scheduleRetry(silent: boolean = false) {
  ...
  ztoolkit.log(`Auto update: Scheduling retry ${autoUpdateRetryCount}/${MAX_RETRIES}`)
...
function finishAutomaticUpdate(errorMessage?: string, silent: boolean = false) {
  ...
  ztoolkit.log(`Auto update: Completed. Updated ${updatedCount}/${totalCount} items`)
}
